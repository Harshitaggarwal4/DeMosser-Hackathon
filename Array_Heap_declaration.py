...
def convertArrToHeap():
    ...
    for line_Index in range (len(lines)):
        ...
        ind1 = -1   # stores teh index where the datatype (if present) is present
        ...
        for newLineI_ind in range(numNewLines):
            ...
            # print(currLine)
            sub = newLineI[newLine_Ind_o+1:newLine_Ind_c:]     #stores the value between the brackets
            # print(sub)
            sub = sub.strip()
            try:
                size = int(sub)
            ...
            newLine = used_DT+"* " + name + " = (" + used_DT + "*) malloc(sizeof(" + used_DT + ")*" + str(size) + ");\n"
